/*
*  Alex Valle S4854159
*  Aritmetica di macchina e stabilità numerica 
*  Laboratorio 1
*/

#include "function.h"


int main(){

    std::cout << "***** Laboratorio 1 - Aritmetica di macchina e stabilità numerica *****" << std::endl;
    std::cout << std::endl;

    //Esegue calcoli e stampa a video i valori dell'esercizio 1
    esercizio_1();

    //Esegue calcoli e stampa a video i valori dell'esercizio 2
    esercizio_2();

    //Esegue calcoli e stampa a video i valori dell'esercizio 3
    std::cout << "\n\nEsercizio 2: " << std::endl;
    std::cout << "esp -> singola precisione = " << espFloat() << std::endl;
    std::cout << "esp -> doppia  precisione = " << espDouble() << std::endl;
}